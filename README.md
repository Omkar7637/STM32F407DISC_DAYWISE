...
### **Step 32: FreeRTOS Message Queues**  

#### **Theory:**  
- **Message Queues** allow tasks to **send and receive data** asynchronously.  
- Used for **task-to-task** or **ISR-to-task** communication.  
- Each queue has a **fixed size** and can store multiple **data items**.  

---

### **Practical: Sending and Receiving Messages using FreeRTOS Queue**  
#### **Objective:**  
- **Task 1** (Sender) sends an **integer** (LED status) to the queue every **1 second**.  
- **Task 2** (Receiver) reads the value and **toggles the LED** accordingly.  

---

### **Steps to Implement:**  
1. **Create a Queue Handle.**  
2. **Define Sender and Receiver Tasks.**  
3. **Create Tasks and Queue in `main()`.**  
4. **Sender Task puts values in the Queue.**  
5. **Receiver Task reads from the Queue and toggles an LED.**  
...
int main(void)
{
    ...
    // Create Queue (stores up to 5 integers)
    myQueue = xQueueCreate(5, sizeof(int));
...
/* Sender Task: Sends LED status to queue */
void SenderTask(void *pvParameters)
{
    int ledStatus = 0;
    ...
    while (1)
    {
        ledStatus = !ledStatus;  // Toggle status (0 or 1)
        xQueueSend(myQueue, &ledStatus, portMAX_DELAY);  // Send to queue
        vTaskDelay(pdMS_TO_TICKS(1000));  // Delay 1 second
    }
}

/* Receiver Task: Reads from queue and toggles LED */
void ReceiverTask(void *pvParameters)
{
    int receivedStatus;
    ...
    while (1)
    {
        if (xQueueReceive(myQueue, &receivedStatus, portMAX_DELAY) == pdTRUE)
        {
            HAL_GPIO_WritePin(LED_PORT, LED_PIN, receivedStatus);
        }
    }
}
...
1. **Create a Queue Handle**  
   ...
   QueueHandle_t myQueue;
   ```
   - Stores **queue instance**.

2. **Create a Queue**  
   ```c
   myQueue = xQueueCreate(5, sizeof(int));
   ```
   - Holds **5 integers** (each **4 bytes**).  
...
3. **Sender Task**  
   ...
   void SenderTask(void *pvParameters)
   {
       ...
       {
           ledStatus = !ledStatus;
           xQueueSend(myQueue, &ledStatus, portMAX_DELAY);
           vTaskDelay(pdMS_TO_TICKS(1000));
       }
   }
   ```
   - Sends **LED status (0 or 1)** to queue **every second**.  
...
4. **Receiver Task**  
   ...
   ```
   - Reads from **queue** and updates **LED state**.  

---

### **Expected Behavior:**  
- **Every 1 second**, the LED **toggles ON/OFF** based on queue data.  
...
| **xQueueReceive()** | Reads data from queue |
| **uxQueueMessagesWaiting()** | Returns the number of items in the queue |
| **vQueueDelete()** | Deletes a queue |

---

## **Next Step: FreeRTOS Mutexes**  


### **Step 33: FreeRTOS Mutexes (Mutual Exclusion Semaphores)**  

#### **Theory:**  
- **Mutex (Mutual Exclusion Semaphore)** prevents multiple tasks from **simultaneously accessing shared resources** (e.g., UART, LCD, I2C).  
- Unlike **binary semaphores**, a **mutex supports priority inheritance**, preventing **priority inversion** issues.  
- A task **locks (takes)** the mutex before accessing the shared resource and **unlocks (gives)** it after use.  

---

### **Practical: Protecting UART with a Mutex**  
#### **Objective:**  
- **Task 1** and **Task 2** both try to **send data over UART**.  
- A **mutex ensures** only one task accesses **UART at a time**.  

---

### **Steps to Implement:**  
1. **Create a Mutex Handle.**  
2. **Define Two Tasks for UART Communication.**  
3. **Create Tasks and Mutex in `main()`.**  
4. **Each Task Locks the Mutex Before Sending Data.**  
5. **Each Task Unlocks the Mutex After Sending Data.**  
...
/* Task 1: Sends "Task 1 Running" over UART */
void Task1(void *pvParameters)
{
    while (1)
    {
...
/* Task 2: Sends "Task 2 Running" over UART */
void Task2(void *pvParameters)
{
    while (1)
    {
...
1. **Create a Mutex Handle**  
   ...
   SemaphoreHandle_t uartMutex;
   ```
   - Stores **mutex instance**.

2. **Create a Mutex**  
   ```c
   uartMutex = xSemaphoreCreateMutex();
   ```
   - Ensures **only one task** accesses **UART at a time**.
...
3. **Task 1 (UART Send)**  
   ...
   ```
   - **Takes mutex**, sends **UART message**, then **releases mutex**.  
...
4. **Task 2 (UART Send)**  
   ...
   ```
   - Same logic as Task 1 but sends **"Task 2 Running"**.
...
- UART displays:
  ...
  Task 2 Running
  ```
- Tasks **take turns** using UART **without conflicts**.
...
| **xSemaphoreCreateMutex()** | Creates a mutex |
| **xSemaphoreTake()** | Locks (takes) the mutex |
| **xSemaphoreGive()** | Unlocks (gives) the mutex |
| **vSemaphoreDelete()** | Deletes the mutex |
...
### **Step 34: FreeRTOS Event Groups**  

#### **Theory:**  
- **Event Groups** allow **multiple tasks** to synchronize based on **event flags**.  
- Unlike semaphores, **multiple bits (flags)** can be set/cleared in a **single variable**.  
- Tasks can **wait for one or multiple events** before proceeding.  

---

### **Practical: LED Control with Event Groups**  
#### **Objective:**  
- Task 1 **sets event flags** when a button is pressed.  
- Task 2 **waits for the event flags** to turn on an LED.  

---

### **Steps to Implement:**  
1. **Create an Event Group Handle.**  
2. **Define Two Tasks:**  
   - **Task 1:** Detects button press and sets event flags.  
   - **Task 2:** Waits for the event flag and toggles an LED.  
3. **Use Event Group API to Set and Wait for Events.**  

---

### **Code for FreeRTOS Event Groups Example**
```c
#include "main.h"
#include "FreeRTOS.h"
#include "task.h"
...
#define EVENT_BUTTON_PRESSED  (1 << 0)  // Event Flag Bit 0
...
int main(void)
{
    HAL_Init();
    SystemClock_Config();
    MX_GPIO_Init();  // Initialize GPIO for button and LED
    ...
    {
        // Create FreeRTOS tasks
        xTaskCreate(Task_Button, "Task_Button", 128, NULL, 1, NULL);
...
/* Task 1: Detects Button Press and Sets Event */
void Task_Button(void *pvParameters)
{
    while (1)
    {
        ...
        {
            xEventGroupSetBits(eventGroup, EVENT_BUTTON_PRESSED);  // Set event flag
            vTaskDelay(pdMS_TO_TICKS(200));  // Debounce delay
        }
    }
}

/* Task 2: Waits for Event and Toggles LED */
void Task_LED(void *pvParameters)
{
    while (1)
    {
        xEventGroupWaitBits(eventGroup, EVENT_BUTTON_PRESSED, pdTRUE, pdFALSE, portMAX_DELAY);
        HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_12);  // Toggle LED
    }
}
...
### **Explanation of Code:**  
1. **Define an Event Group Handle**  
   ```c
   EventGroupHandle_t eventGroup;
   ```
   - Stores event group instance.

2. **Create an Event Group**  
   ```c
   eventGroup = xEventGroupCreate();
   ```
   - Allocates memory for event flags.

3. **Task 1 (Button Press Detection)**  
   ```c
   if (HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13) == GPIO_PIN_RESET)
   {
       xEventGroupSetBits(eventGroup, EVENT_BUTTON_PRESSED);
   }
   ```
   - Detects button press and **sets event bit**.

4. **Task 2 (LED Control)**  
   ```c
   xEventGroupWaitBits(eventGroup, EVENT_BUTTON_PRESSED, pdTRUE, pdFALSE, portMAX_DELAY);
   HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_12);
   ```
   - **Waits** for event flag, then toggles LED.

---

### **Expected Behavior:**  
- When **button is pressed**, LED **toggles**.  
- **No semaphore needed**, as multiple events can be managed.
...
|-------------|-------------|
| **xEventGroupCreate()** | Creates an event group |
| **xEventGroupSetBits()** | Sets event flags |
| **xEventGroupWaitBits()** | Waits for specific event flags |
| **xEventGroupClearBits()** | Clears event flags |
| **xEventGroupDelete()** | Deletes the event group |
...
### **Step 35: FreeRTOS Software Timers**  

#### **Theory:**  
- **Software Timers** allow **delayed or periodic execution** of tasks without blocking CPU time.  
- Unlike hardware timers, FreeRTOS **manages these timers using the scheduler**.  
- Useful for **delayed actions, periodic tasks, and timeout handling**.  

---

### **Practical: LED Blinking with Software Timer**  
#### **Objective:**  
- Create a **software timer** that toggles an LED every **500ms**.  
...
1. **Create a Timer Handle.**  
2. **Define a Timer Callback Function.**  
3. **Initialize and Start the Timer.**  
4. **Configure LED GPIO in STM32.**  
...
#define TIMER_PERIOD pdMS_TO_TICKS(500)  // 500ms timer

TimerHandle_t ledTimer;  // Timer Handle
...
int main(void)
{
    ...
    ledTimer = xTimerCreate("LED_Timer", TIMER_PERIOD, pdTRUE, 0, TimerCallback);

    if (ledTimer != NULL)
    {
        xTimerStart(ledTimer, 0);  // Start timer
        vTaskStartScheduler();  // Start FreeRTOS scheduler
    }
...
1. **Create a Timer Handle**  
   ...
   TimerHandle_t ledTimer;
   ```
   - Stores the software timer instance.

2. **Define the Timer Callback Function**  
   ```c
   void TimerCallback(TimerHandle_t xTimer)
   {
       HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_12);  // Toggle LED
   ...
   ```
   - This function executes every **500ms** to toggle an LED.

3. **Create and Start the Timer**  
   ```c
   ledTimer = xTimerCreate("LED_Timer", TIMER_PERIOD, pdTRUE, 0, TimerCallback);
   ```
   - Creates a timer with **500ms** period and calls `TimerCallback` on expiration.
...
### **Expected Behavior:**  
- The LED will **toggle every 500ms** using a **software timer**.  
...
| **xTimerReset()** | Resets the timer |
| **xTimerChangePeriod()** | Changes the timer interval |
| **xTimerDelete()** | Deletes the timer |
...
### **Step 36: FreeRTOS Message Queues**  

#### **Theory:**  
- **Message Queues** allow tasks to **communicate** by sending and receiving messages.  
- Useful for **data exchange between tasks** or between **interrupts and tasks**.  
- Each queue has **a fixed size** and stores **data structures or simple values**.  

---

### **Practical: LED Control Using Message Queue**  
#### **Objective:**  
- Task **1:** Reads a button press and sends a message.  
- Task **2:** Receives the message and toggles an LED.  
...
1. **Create a Queue Handle.**  
2. **Define Sender and Receiver Tasks.**  
3. **Send a Message when the Button is Pressed.**  
4. **Receive the Message and Toggle LED.**  
...
QueueHandle_t ledQueue;  // Queue Handle
...
int main(void)
{
    ...
    // Create Queue (stores 5 integer values)
    ledQueue = xQueueCreate(5, sizeof(uint8_t));

    if (ledQueue != NULL)
    {
        xTaskCreate(SenderTask, "ButtonTask", 128, NULL, 1, NULL);
        xTaskCreate(ReceiverTask, "LEDTask", 128, NULL, 1, NULL);
...
/* Task 1: Send Message on Button Press */
void SenderTask(void *pvParameters)
{
    uint8_t msg = 1;
    ...
    while (1)
    {
        ...
        {
            xQueueSend(ledQueue, &msg, portMAX_DELAY);  // Send Message
...
/* Task 2: Receive Message and Toggle LED */
void ReceiverTask(void *pvParameters)
{
    uint8_t msg;
...
1. **Create a Queue Handle**  
   ...
   ```
   - Stores the queue instance.

2. **Create a Queue**  
   ```c
   ledQueue = xQueueCreate(5, sizeof(uint8_t));
   ```
   - Creates a queue that can store **5 messages** of **1-byte (uint8_t) size**.

3. **Sender Task (Button Press)**  
   ```c
   ...
   ```
   - Reads **button state** and **sends a message (1)** to the queue.

4. **Receiver Task (Toggle LED)**  
   ```c
   if (xQueueReceive(ledQueue, &msg, portMAX_DELAY) == pdPASS)
   {
       HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_12);
   ...
   ```
   - **Waits for a message** and **toggles the LED** when received.

---

### **Expected Behavior:**  
- **Pressing the button** will **toggle the LED** using the queue.  
...
| **xQueueReceive()** | Reads a message from the queue |
| **uxQueueMessagesWaiting()** | Checks the number of messages in the queue |
...
### **Step 37: FreeRTOS Mutexes (Mutual Exclusion)**  

#### **Theory:**  
- **Mutex (Mutual Exclusion)** prevents multiple tasks from accessing a shared resource **at the same time**.  
- Unlike semaphores, a mutex is **owned by the task** that takes it and must be **released by the same task**.  
- Used for **protecting shared resources** like **UART, SPI, I2C, and global variables**.  

---

### **Practical: UART Printing Using Mutex**  
#### **Objective:**  
- Task **1:** Prints `"Task 1 Running"` every second.  
- Task **2:** Prints `"Task 2 Running"` every two seconds.  
- **Mutex ensures** only **one task accesses UART** at a time.  
...
### **Steps to Implement:**  
1. **Create a Mutex Handle.**  
2. **Define Two Tasks that Share UART.**  
3. **Lock the Mutex before using UART.**  
4. **Release the Mutex after transmission.**  
...
1. **Create a Mutex Handle**  
   ...
   SemaphoreHandle_t uartMutex;
   ```
   - Defines a **mutex for UART access**.
...
3. **Task 1 (Print Message)**  
   ...
   ```
   - **Takes the mutex**, **prints a message**, and **releases it**.

4. **Task 2 (Print Message)**  
   ```c
   if (xSemaphoreTake(uartMutex, portMAX_DELAY) == pdTRUE)
   {
       char msg[] = "Task 2 Running\r\n";
...
- **No message conflicts in UART output.**  
...
|-------------|-------------|
| **xSemaphoreCreateMutex()** | Creates a mutex |
| **xSemaphoreTake()** | Locks the mutex |
| **xSemaphoreGive()** | Releases the mutex |
| **uxSemaphoreGetCount()** | Checks mutex state |
...
### **Step 38: FreeRTOS Event Groups**  

#### **Theory:**  
- **Event Groups** allow multiple tasks to **wait for multiple events** (bits) simultaneously.  
- They are useful for **task synchronization**, where tasks must wait for specific conditions.  
- Each **bit** in an **event group** represents a different event.  
- Tasks can **set**, **clear**, and **wait for** event bits.  

---

### **Practical: LED Control Using Event Groups**  
#### **Objective:**  
- **Task 1:** Waits for an event and **turns ON LED** when triggered.  
- **Task 2:** Triggers the event every **2 seconds**.  
- Uses **Event Groups for synchronization**.  
...
1. **Create an Event Group.**  
2. **Define Two Tasks.**  
3. **Task 1 waits for an event to turn ON the LED.**  
4. **Task 2 sets the event every 2 seconds.**  
...
void LED_Task(void *pvParameters);
void Event_Trigger_Task(void *pvParameters);
...
int main(void)
{
    ...
    {
        xTaskCreate(LED_Task, "LED Task", 128, NULL, 1, NULL);
        xTaskCreate(Event_Trigger_Task, "Trigger Task", 128, NULL, 1, NULL);
...
/* Task 1: Wait for event to turn ON LED */
void LED_Task(void *pvParameters)
{
    while (1)
    {
...
/* Task 2: Set event every 2 seconds */
void Event_Trigger_Task(void *pvParameters)
{
    while (1)
    {
        vTaskDelay(pdMS_TO_TICKS(2000));  // 2 sec delay
        xEventGroupSetBits(eventGroup, EVENT_BIT);  // Set event bit
    }
}
...
2. **Define an Event Bit**  
   ```c
   #define EVENT_BIT (1 << 0)
   ```
   - Uses **bit 0** for LED control.

3. **Task 1: Wait for Event**  
   ```c
   xEventGroupWaitBits(eventGroup, EVENT_BIT, pdTRUE, pdFALSE, portMAX_DELAY);
   HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);
   ```
   - **Waits for event bit**.
   - **Toggles LED** when event occurs.

4. **Task 2: Set Event Bit**  
   ```c
   vTaskDelay(pdMS_TO_TICKS(2000));
   xEventGroupSetBits(eventGroup, EVENT_BIT);
   ```
   - **Sets event bit** every **2 seconds**.

---

### **Expected Behavior:**  
- **LED toggles every 2 seconds** when the event is triggered.  
...
| **xEventGroupSetBits()** | Sets event bits |
| **xEventGroupClearBits()** | Clears event bits |
| **xEventGroupWaitBits()** | Waits for specific event bits |
...
### **Step 39: FreeRTOS Message Queues**  

#### **Theory:**  
- **Message Queues** allow tasks to communicate by **sending and receiving messages**.  
- Messages are stored in a **queue buffer** and can be accessed **FIFO (First In, First Out)**.  
- Used when tasks need to **exchange small data** (e.g., sensor values, commands).  

---

### **Practical: Send Sensor Data via Queue**  
#### **Objective:**  
- **Task 1:** Simulates **reading a sensor value** and sends it via a queue.  
- **Task 2:** Receives data from the queue and **prints it via UART**.  

---

### **Steps to Implement:**  
1. **Create a Message Queue.**  
2. **Task 1 sends sensor data to the queue.**  
3. **Task 2 receives data and prints via UART.**  
...
int main(void)
{
    ...
    if (sensorQueue != NULL)
    {
        xTaskCreate(Sensor_Task, "Sensor Task", 128, NULL, 1, NULL);
        xTaskCreate(UART_Task, "UART Task", 128, NULL, 1, NULL);
...
void Sensor_Task(void *pvParameters)
{
    ...
    while (1)
    {
        sensorValue += 10;  // Simulate sensor value
        xQueueSend(sensorQueue, &sensorValue, portMAX_DELAY);  // Send data to queue
        vTaskDelay(pdMS_TO_TICKS(1000));  // 1 sec delay
    }
}

/* Task 2: Receive Sensor Data and Print */
void UART_Task(void *pvParameters)
{
    int receivedValue;
    char buffer[20];
...
1. **Create a Queue**  
   ...
   sensorQueue = xQueueCreate(5, sizeof(int));
   ```
   - Creates a queue to hold **5 integers**.

2. **Task 1: Send Data to Queue**  
   ```c
   sensorValue += 10;
   xQueueSend(sensorQueue, &sensorValue, portMAX_DELAY);
   ```
   - **Sends sensor value** to the queue every **1 second**.

3. **Task 2: Receive Data and Print**  
   ```c
   if (xQueueReceive(sensorQueue, &receivedValue, portMAX_DELAY) == pdPASS)
   {
       sprintf(buffer, "Value: %d\r\n", receivedValue);
...
### **Expected Behavior:**  
- **Every second, UART prints a new sensor value**.  
...
## **Next Step: FreeRTOS Timers**  


### **Step 40: FreeRTOS Timers**  

#### **Theory:**  
- **FreeRTOS Software Timers** allow executing functions **after a specific time interval**.  
- Unlike hardware timers, these **run in software**, managed by the **FreeRTOS timer task**.  
- Timers can be **one-shot** (execute once) or **periodic** (execute repeatedly).  

---

### **Practical: Blink LED Using FreeRTOS Timer**  
#### **Objective:**  
- **Create a FreeRTOS timer** to **toggle an LED** every **500ms**.  

---

### **Steps to Implement:**  
1. **Create a software timer.**  
2. **Set the timer to trigger every 500ms.**  
3. **When the timer expires, toggle the LED.**  

---

### **Code for FreeRTOS Timer Example**
```c
#include "main.h"
#include "FreeRTOS.h"
#include "task.h"
...
1. **Create a Software Timer**  
   ...
   ledTimer = xTimerCreate("LED Timer", pdMS_TO_TICKS(500), pdTRUE, 0, LED_Timer_Callback);
   ```
   - **Creates a timer** named `"LED Timer"`.  
   - **Period:** 500ms.  
   - **pdTRUE:** Auto-reloads (runs periodically).  
   - **Callback Function:** `LED_Timer_Callback()`  
...
2. **Start the Timer**  
   ...
   xTimerStart(ledTimer, 0);
   ```
   - **Starts the timer**, making it execute every 500ms.

3. **Toggle LED in Timer Callback**  
   ```c
   HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_12);
   ```
   - **Toggles the LED** when the timer expires.

---

### **Expected Behavior:**  
- **The LED blinks every 500ms** using a **software timer**.  
...
| **xTimerStop()** | Stops the timer |
| **xTimerReset()** | Resets the timer |
| **xTimerChangePeriod()** | Changes the timer period |
...
### **Step 41: FreeRTOS Event Groups**  

#### **Theory:**  
- **Event Groups** allow multiple tasks to communicate **using event flags**.  
- A **task can wait** for specific **bits (events)** to be set in an event group.  
- Event groups are useful for **task synchronization**.  

---

### **Practical: Synchronizing Tasks Using Event Groups**  
#### **Objective:**  
- **Task 1:** Sets an event bit every **1 second**.  
- **Task 2:** Waits for the event bit and then **toggles an LED**.  
...
### **Steps to Implement:**  
1. **Create an Event Group.**  
2. **Task 1 sets a bit in the event group.**  
3. **Task 2 waits for the bit and toggles the LED.**  
...
1. **Create an Event Group**  
   ...
   eventGroup = xEventGroupCreate();
   ```
   - **Creates an event group** that both tasks can use.

2. **Task 1 Sets Event Bit Every 1 Second**  
   ```c
   xEventGroupSetBits(eventGroup, EVENT_BIT);
   ```
   - **Sets the event bit** to notify Task 2.

3. **Task 2 Waits for Event Bit**  
   ```c
   xEventGroupWaitBits(eventGroup, EVENT_BIT, pdTRUE, pdFALSE, portMAX_DELAY);
   ```
   - **Waits for the event bit.**  
   - Once set, **toggles an LED**.

---

### **Expected Behavior:**  
- **Task 1 sets an event every second.**  
- **Task 2 waits for the event and toggles an LED.**  
...
|-------------|-------------|
| **xEventGroupCreate()** | Creates an event group |
| **xEventGroupSetBits()** | Sets specific event bits |
| **xEventGroupWaitBits()** | Waits for event bits |
| **xEventGroupClearBits()** | Clears specific bits |

---

## **Next Step: FreeRTOS Queues**  


### **Step 42: FreeRTOS Queues**  

#### **Theory:**  
- A **queue** is a FIFO (First In, First Out) data structure used in FreeRTOS.  
- It allows **tasks to send and receive data asynchronously**.  
- Useful for **inter-task communication** (e.g., **sensor data processing**).  

---

### **Practical: Sending Data Between Two Tasks Using a Queue**  
#### **Objective:**  
- **Task 1:** Sends an **integer value** to the queue every 500ms.  
- **Task 2:** Receives the value from the queue and **toggles an LED** based on the received data.  

---

### **Steps to Implement:**  
1. **Create a Queue.**  
2. **Task 1 sends data to the queue.**  
3. **Task 2 reads data from the queue and controls an LED.**  
...
int main(void)
{
    ...
    myQueue = xQueueCreate(5, sizeof(int));  // Create Queue (5 elements, int size)

    xTaskCreate(Task1_Send, "Task1", 128, NULL, 1, NULL);
    xTaskCreate(Task2_Receive, "Task2", 128, NULL, 1, NULL);
...
void Task1_Send(void *pvParameters)
{
    ...
    while (1)
    {
        xQueueSend(myQueue, &count, portMAX_DELAY);  // Send data to Queue
        count++;  // Increment count
        vTaskDelay(pdMS_TO_TICKS(500));  // Delay 500ms
    }
}

/* Task 2: Receive Data from Queue and Toggle LED */
void Task2_Receive(void *pvParameters)
{
    int receivedValue;
    while (1)
    {
        ...
        {
            if (receivedValue % 2 == 0)  // If received number is even
            {
                HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_12);  // Toggle LED
            }
        }
...
1. **Create a Queue**  
   ...
   myQueue = xQueueCreate(5, sizeof(int));
   ```
   - **Creates a queue** with a **capacity of 5 integers**.

2. **Task 1 Sends Data**  
   ```c
   xQueueSend(myQueue, &count, portMAX_DELAY);
   ```
   - Sends an **incrementing count value** to the queue **every 500ms**.

3. **Task 2 Receives Data**  
   ```c
   xQueueReceive(myQueue, &receivedValue, portMAX_DELAY);
   ```
   - Reads the data from the queue.  
   - If the **received number is even**, toggles an LED.

---

### **Expected Behavior:**  
- **Task 1 sends numbers to the queue every 500ms.**  
- **Task 2 reads the numbers and toggles an LED when it receives an even number.**  
...
| **xQueueSend()** | Sends data to a queue |
| **xQueueReceive()** | Receives data from a queue |
| **uxQueueSpacesAvailable()** | Checks available space in queue |
...
### **Step 43: FreeRTOS Mutexes**  

#### **Theory:**  
- **Mutex (Mutual Exclusion)** is a synchronization mechanism used in FreeRTOS to **prevent data corruption** when multiple tasks access shared resources.  
- Unlike binary semaphores, **mutexes support priority inheritance**, which prevents priority inversion.  

---

### **Practical: Using a Mutex to Protect Shared Resource**  
#### **Objective:**  
- Two tasks **attempt to write to UART**.  
- A **mutex ensures only one task** writes at a time to avoid data corruption.  

---

### **Steps to Implement:**  
1. **Create a Mutex** before the scheduler starts.  
2. **Two tasks attempt to write** to UART.  
3. **Mutex ensures exclusive access** to UART.  
...
int main(void)
{
    ...
    xTaskCreate(Task1_UART, "Task1", 128, NULL, 1, NULL);
    xTaskCreate(Task2_UART, "Task2", 128, NULL, 1, NULL);
...
void Task1_UART(void *pvParameters)
{
    while (1)
    {
        if (xSemaphoreTake(uartMutex, portMAX_DELAY) == pdTRUE)  // Acquire Mutex
        {
            char msg[] = "Task 1 Writing to UART\n";
            HAL_UART_Transmit(&huart2, (uint8_t *)msg, sizeof(msg), HAL_MAX_DELAY);
            xSemaphoreGive(uartMutex);  // Release Mutex
        }
        vTaskDelay(pdMS_TO_TICKS(1000));  // Delay 1 sec
...
void Task2_UART(void *pvParameters)
{
    while (1)
    {
        if (xSemaphoreTake(uartMutex, portMAX_DELAY) == pdTRUE)  // Acquire Mutex
        {
            char msg[] = "Task 2 Writing to UART\n";
            HAL_UART_Transmit(&huart2, (uint8_t *)msg, sizeof(msg), HAL_MAX_DELAY);
            xSemaphoreGive(uartMutex);  // Release Mutex
        }
        vTaskDelay(pdMS_TO_TICKS(1000));  // Delay 1 sec
...
1. **Create a Mutex**  
   ...
   uartMutex = xSemaphoreCreateMutex();
   ```
   - Creates a **mutex for UART communication**.

2. **Task 1 and Task 2 Attempt to Write to UART**  
   ```c
   if (xSemaphoreTake(uartMutex, portMAX_DELAY) == pdTRUE)
   ```
   - **Tries to acquire the mutex.**  
   - If successful, **writes to UART**.

3. **Release the Mutex After Writing**  
   ```c
   xSemaphoreGive(uartMutex);
   ```
   - **Releases the mutex** so other tasks can access UART.

---

### **Expected Behavior:**  
- **Task 1 and Task 2 write to UART without overlapping.**  
- **Mutex ensures one task writes at a time**, avoiding data corruption.
...
|-------------|-------------|
| **xSemaphoreCreateMutex()** | Creates a mutex |
| **xSemaphoreTake()** | Locks the mutex (acquires access) |
| **xSemaphoreGive()** | Unlocks the mutex (releases access) |
...
### **Step 44: FreeRTOS Event Groups**  

#### **Theory:**  
- **Event Groups** in FreeRTOS allow multiple tasks to **wait for specific events** to occur.  
- They are useful for **synchronizing multiple tasks** using **bit flags** instead of queues or semaphores.  
- Each **bit in an Event Group** represents an event.  

---

### **Practical: Using Event Groups for Task Synchronization**  
#### **Objective:**  
- Two tasks **wait for different events**.  
- Another task **sets event flags** to trigger actions.  

---

### **Steps to Implement:**  
1. **Create an Event Group** before the scheduler starts.  
2. **Task 1 waits for EVENT_1**, and **Task 2 waits for EVENT_2**.  
3. **Task 3 sets event flags** at different times.  
...
#define EVENT_1 (1 << 0)  // Bit 0
#define EVENT_2 (1 << 1)  // Bit 1

EventGroupHandle_t eventGroup;  // Event Group Handle

void Task1_WaitEvent(void *pvParameters);
void Task2_WaitEvent(void *pvParameters);
void Task3_SetEvent(void *pvParameters);
...
int main(void)
{
    ...
    xTaskCreate(Task1_WaitEvent, "Task1", 128, NULL, 1, NULL);
    xTaskCreate(Task2_WaitEvent, "Task2", 128, NULL, 1, NULL);
    xTaskCreate(Task3_SetEvent, "Task3", 128, NULL, 2, NULL);
...
/* Task 1: Waits for EVENT_1 */
void Task1_WaitEvent(void *pvParameters)
{
    while (1)
    {
        xEventGroupWaitBits(eventGroup, EVENT_1, pdTRUE, pdFALSE, portMAX_DELAY);
        char msg[] = "Task 1: EVENT_1 Received!\n";
        HAL_UART_Transmit(&huart2, (uint8_t *)msg, sizeof(msg), HAL_MAX_DELAY);
    }
}

/* Task 2: Waits for EVENT_2 */
void Task2_WaitEvent(void *pvParameters)
{
    while (1)
    {
        xEventGroupWaitBits(eventGroup, EVENT_2, pdTRUE, pdFALSE, portMAX_DELAY);
        char msg[] = "Task 2: EVENT_2 Received!\n";
        HAL_UART_Transmit(&huart2, (uint8_t *)msg, sizeof(msg), HAL_MAX_DELAY);
    }
}
...
void Task3_SetEvent(void *pvParameters)
{
    while (1)
    {
        ...
        xEventGroupSetBits(eventGroup, EVENT_1);

        vTaskDelay(pdMS_TO_TICKS(3000));  // Delay 3 sec
        xEventGroupSetBits(eventGroup, EVENT_2);
    }
}
...
2. **Task 1 and Task 2 Wait for Different Events**  
   ```c
   xEventGroupWaitBits(eventGroup, EVENT_1, pdTRUE, pdFALSE, portMAX_DELAY);
   ```
   - **Task 1 waits for EVENT_1**.
   - When received, **prints "Task 1: EVENT_1 Received!"**.

   ```c
   xEventGroupWaitBits(eventGroup, EVENT_2, pdTRUE, pdFALSE, portMAX_DELAY);
   ```
   - **Task 2 waits for EVENT_2**.
   - When received, **prints "Task 2: EVENT_2 Received!"**.

3. **Task 3 Triggers Events at Different Times**  
   ```c
   vTaskDelay(pdMS_TO_TICKS(2000));
   xEventGroupSetBits(eventGroup, EVENT_1);
   ```
   - After **2 seconds**, Task 3 **sets EVENT_1**.

   ```c
   vTaskDelay(pdMS_TO_TICKS(3000));
   xEventGroupSetBits(eventGroup, EVENT_2);
   ```
   - After **another 3 seconds**, Task 3 **sets EVENT_2**.

---

### **Expected Behavior:**  
- **Task 1 prints a message when EVENT_1 is set (after 2 sec).**  
- **Task 2 prints a message when EVENT_2 is set (after 5 sec).**  
- **Task 3 controls when each event occurs.**  
...
### **Step 45: FreeRTOS Software Timers**  

#### **Theory:**  
- **Software timers** allow tasks to execute **periodically or after a delay** without blocking CPU time.  
- They run in the **background using the FreeRTOS timer service task**.  
- Useful for **watchdogs, periodic status checks, or event-triggered actions**.  

---

### **Practical: Using a FreeRTOS Software Timer**  
#### **Objective:**  
- **Toggle an LED every 1 second** using a **software timer** instead of a task delay.  

---

### **Steps to Implement:**  
1. **Create a Software Timer** before the scheduler starts.  
2. **Define a Timer Callback Function** to toggle the LED.  
3. **Start the Timer** in the main function.  
...
int main(void)
{
    ...
    // Create a Timer that runs every 1000 ms (1 sec), auto-reloads
    myTimerHandle = xTimerCreate("LED_Timer", pdMS_TO_TICKS(1000), pdTRUE, (void *)0, TimerCallback);
...
void TimerCallback(TimerHandle_t xTimer)
{
    HAL_GPIO_TogglePin(LED_PORT, LED_PIN);  // Toggle LED every 1 sec
}
...
1. **Create a Software Timer**  
   ...
   myTimerHandle = xTimerCreate("LED_Timer", pdMS_TO_TICKS(1000), pdTRUE, (void *)0, TimerCallback);
   ```
   - Creates a **1-second software timer** named `"LED_Timer"`.  
   - `pdTRUE` means **auto-reload** (keeps running indefinitely).  
   - Calls `TimerCallback()` every **1000 ms (1 sec)**.  

2. **Start the Timer**  
   ```c
   xTimerStart(myTimerHandle, 0);
   ```
   - **Starts the software timer** before the scheduler runs.  
...
### **Expected Behavior:**  
- The LED **blinks every 1 second** using a software timer.  
- No need to create a dedicated task with `vTaskDelay()`.  
...
## **Next Step: FreeRTOS Message Buffers**  


### **Step 46: FreeRTOS Message Buffers**  

#### **Theory:**  
- **Message Buffers** allow tasks to send and receive **variable-length data** efficiently.  
- Unlike queues (which store fixed-size messages), message buffers can handle **dynamic-length messages**.  
- Used for **data logging, sensor data transmission, or UART communication**.  

---

### **Practical: Using FreeRTOS Message Buffers**  
#### **Objective:**  
- **Task 1:** Sends "Hello from Task 1!" to a **Message Buffer**.  
- **Task 2:** Receives and prints the message via **UART**.  

---

### **Steps to Implement:**  
1. **Create a Message Buffer** before starting the scheduler.  
2. **Task 1 writes** a message into the buffer.  
3. **Task 2 reads** from the buffer and sends data over UART.  

---

### **Code for FreeRTOS Message Buffer**
```c
#include "main.h"
#include "FreeRTOS.h"
#include "task.h"
...
#define BUFFER_SIZE 50  // Define Message Buffer Size

MessageBufferHandle_t myMessageBuffer;  // Message Buffer Handle
...
int main(void)
{
    ...
    myMessageBuffer = xMessageBufferCreate(BUFFER_SIZE);

    if (myMessageBuffer != NULL)
    {
        // Create Two Tasks
        xTaskCreate(Task1_Send, "Task1", 128, NULL, 1, NULL);
        xTaskCreate(Task2_Receive, "Task2", 128, NULL, 1, NULL);
...
void Task1_Send(void *pvParameters)
{
    char message[] = "Hello from Task 1!";
    
    while (1)
    {
        xMessageBufferSend(myMessageBuffer, message, strlen(message) + 1, portMAX_DELAY);
        vTaskDelay(pdMS_TO_TICKS(2000));  // Wait for 2 seconds
    }
}
...
void Task2_Receive(void *pvParameters)
{
    ...
    while (1)
    {
        size_t bytesRead = xMessageBufferReceive(myMessageBuffer, receivedMessage, BUFFER_SIZE, portMAX_DELAY);

        if (bytesRead > 0)
        {
            UART_SendString(receivedMessage);
        }
    }
}
...
1. **Create a Message Buffer**  
   ...
   myMessageBuffer = xMessageBufferCreate(BUFFER_SIZE);
   ```
   - Allocates a **50-byte message buffer**.  

2. **Task 1 Sends Data**  
   ```c
   xMessageBufferSend(myMessageBuffer, message, strlen(message) + 1, portMAX_DELAY);
   ```
   - Sends `"Hello from Task 1!"` to the **message buffer**.  
   - `portMAX_DELAY` ensures it waits until space is available.  

3. **Task 2 Reads Data**  
   ```c
   size_t bytesRead = xMessageBufferReceive(myMessageBuffer, receivedMessage, BUFFER_SIZE, portMAX_DELAY);
   ```
   - Reads from the **message buffer** and prints it via **UART**.  

4. **UART Output**  
   - If Task 2 receives `"Hello from Task 1!"`, it prints:  
     ```
     Hello from Task 1!
     ```
...
### **Expected Behavior:**  
- Every **2 seconds**, Task 1 sends a message.  
- Task 2 **receives and prints** it over **UART2**.  

---

### **Summary of FreeRTOS Message Buffer Functions**  
| **Function** | **Purpose** |
|-------------|-------------|
| **xMessageBufferCreate()** | Creates a message buffer |
| **xMessageBufferSend()** | Sends a message to the buffer |
| **xMessageBufferReceive()** | Reads a message from the buffer |

---

## **Next Step: FreeRTOS Stream Buffers**  


### **Step 47: FreeRTOS Stream Buffers**

#### **Theory:**
- **Stream Buffers** are similar to **Message Buffers** but are designed for continuous streams of data, making them ideal for data **streaming applications** like **audio or video data**.
- They allow **tasks** to send and receive **byte-by-byte data** in a **streaming fashion** rather than in blocks, as in message buffers.

#### **Key Points:**
- Stream buffers are useful for **real-time data** processing and communication.
- They are faster for handling **byte-oriented data** in scenarios like **sensor readings** or **data packets**.
- **Stream Buffers** differ from **Message Buffers** because they store and retrieve **data byte by byte**.

---

### **Practical: Using FreeRTOS Stream Buffers**
#### **Objective:**
- **Task 1** will send a **byte stream** to a **Stream Buffer**.
- **Task 2** will **receive** and **print** this stream.

---

### **Steps to Implement:**
1. **Create a Stream Buffer** before the scheduler starts.
2. **Task 1 writes** byte-by-byte data into the stream buffer.
3. **Task 2 reads** the data from the stream buffer and sends it over **UART**.

---

### **Code for FreeRTOS Stream Buffer**
```c
#include "main.h"
#include "FreeRTOS.h"
#include "task.h"
...
#define STREAM_BUFFER_SIZE 100  // Define Stream Buffer Size

StreamBufferHandle_t myStreamBuffer;  // Stream Buffer Handle
...
int main(void)
{
    ...
    myStreamBuffer = xStreamBufferCreate(STREAM_BUFFER_SIZE, 1);

    if (myStreamBuffer != NULL)
    {
        // Create Two Tasks
        xTaskCreate(Task1_Send, "Task1", 128, NULL, 1, NULL);
        xTaskCreate(Task2_Receive, "Task2", 128, NULL, 1, NULL);
...
/* Task 1: Sends Byte Stream */
void Task1_Send(void *pvParameters)
{
    char message[] = "FreeRTOS Stream Buffer Test!";  // Message to send
    size_t bytesWritten;

    while (1)
    {
        bytesWritten = xStreamBufferSend(myStreamBuffer, message, strlen(message), portMAX_DELAY);
        if (bytesWritten > 0)
        {
...
/* Task 2: Receives Byte Stream */
void Task2_Receive(void *pvParameters)
{
    char receivedMessage[STREAM_BUFFER_SIZE];
    size_t bytesRead;

    while (1)
    {
        bytesRead = xStreamBufferReceive(myStreamBuffer, receivedMessage, STREAM_BUFFER_SIZE, portMAX_DELAY);
...
1. **Create a Stream Buffer**  
   ```c
   myStreamBuffer = xStreamBufferCreate(STREAM_BUFFER_SIZE, 1);
   ```
   - Creates a **Stream Buffer** with a **size of 100 bytes**.

2. **Task 1 Sends Data**  
   ```c
   bytesWritten = xStreamBufferSend(myStreamBuffer, message, strlen(message), portMAX_DELAY);
   ```
   - Sends a **message byte-by-byte** to the **stream buffer**.
   - Uses `portMAX_DELAY` to block until there is space in the buffer.

3. **Task 2 Reads Data**  
   ```c
   bytesRead = xStreamBufferReceive(myStreamBuffer, receivedMessage, STREAM_BUFFER_SIZE, portMAX_DELAY);
   ```
   - Receives the **data byte-by-byte** from the **stream buffer** and then sends it to the UART.

4. **UART Output**  
   - If **Task 2** receives `"FreeRTOS Stream Buffer Test!"`, it prints:
     ```
     Message Received: FreeRTOS Stream Buffer Test!
     ```

---

### **Expected Behavior:**
- **Task 1** sends a byte stream every **2 seconds**.
- **Task 2** **receives and prints** it over **UART2**.

---

### **FreeRTOS Stream Buffer Functions:**
| **Function** | **Purpose** |
|--------------|-------------|
| **xStreamBufferCreate()** | Creates a stream buffer |
| **xStreamBufferSend()** | Sends data byte-by-byte to the buffer |
| **xStreamBufferReceive()** | Receives data byte-by-byte from the buffer |

---

### **Summary:**
- **Stream Buffers** are used to transfer continuous **byte-oriented data** between tasks.
- This implementation shows how to send and receive byte streams using **FreeRTOS Stream Buffers**.

---

## **Next Step: FreeRTOS Timer Services**  


### **Step 48: FreeRTOS Timer Services**

#### **Theory:**
- **FreeRTOS Timers** provide a way to execute functions at a later time or after a specified delay.
- These timers can be **one-shot** or **periodic**, and they are used to execute callback functions in response to time-based events.

#### **Key Points:**
- **Timers** run in the background and execute a function when the specified time elapses.
- Useful for scenarios like **task delays**, **periodic updates**, or **timeouts**.
- There are **software timers** and **hardware timers** in FreeRTOS.

---

### **Practical: Using FreeRTOS Software Timers**
#### **Objective:**
- Set up a **FreeRTOS Software Timer** that toggles an **LED** every **1 second**.

---

### **Steps to Implement:**
1. **Create the Timer** before the scheduler starts.
2. **Define the callback function** that toggles an LED.
3. **Start the timer** in the main function.
4. **Configure the LED toggling** inside the timer callback.
...
TimerHandle_t myTimer;  // Timer Handle
...
int main(void)
{
    ...
    // Create a Timer (1 second interval, no auto-reload)
    myTimer = xTimerCreate("LED_Timer", pdMS_TO_TICKS(1000), pdTRUE, (void*)0, TimerCallback);
...
/* Timer Callback Function: Toggles the LED */
void TimerCallback(TimerHandle_t xTimer)
{
...
void MX_GPIO_Init(void)
{
    ...
    GPIO_InitTypeDef GPIO_InitStruct = {0};
    GPIO_InitStruct.Pin = GPIO_PIN_5;  // LED connected to pin PA5
...
void SystemClock_Config(void)
{
    // Auto-generated system clock configuration
}
```
...
1. **Timer Creation:**  
   ...
   myTimer = xTimerCreate("LED_Timer", pdMS_TO_TICKS(1000), pdTRUE, (void*)0, TimerCallback);
   ```
   - Creates a **1-second periodic timer**. 
   - The callback function `TimerCallback` will be executed every second.
   - `pdTRUE` specifies the timer is **periodic** (it restarts after each callback).

2. **Starting the Timer:**  
   ```c
   xTimerStart(myTimer, 0);
   ```
   - Starts the timer. The timer will now trigger every 1 second.
...
3. **Timer Callback:**  
   ...
   ```
   - This callback toggles the **LED connected to pin PA5** every time the timer expires.

4. **GPIO Initialization:**  
   ```c
   HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);
   ```
   - This toggles the LED on pin **PA5** (on the **STM32F407 DISC board**).

---

### **Expected Behavior:**
- Every **1 second**, the **timer callback** toggles the **LED** state (on/off).
- This is useful for periodic tasks like **blinking LEDs**, **updating displays**, or **checking sensors**.
...
| **Function**           | **Purpose**                                                                 |
|------------------------|-----------------------------------------------------------------------------|
| **xTimerCreate()**      | Creates a software timer with a specified period and callback function.      |
| **xTimerStart()**       | Starts a previously created timer.                                          |
| **xTimerStop()**        | Stops a running timer.                                                      |
| **xTimerDelete()**      | Deletes a timer (if no longer needed).                                       |
| **xTimerReset()**       | Resets a timer (restarts its countdown).                                     |

---

### **Summary:**
- **FreeRTOS Timers** provide a way to execute tasks periodically or after a delay.
- This example shows how to **toggle an LED** every **second** using a **software timer**.
- You can use these timers for **task scheduling**, **timeouts**, or **periodic updates** in embedded systems.

---

### **Next Step: FreeRTOS Queue Functions**


### **Step 49: FreeRTOS Queue Functions**

#### **Theory:**
- A **queue** in FreeRTOS is a **data structure** used to **pass data** between tasks or between interrupt and task.
- Tasks can **send** or **receive** data through queues, making them crucial for **inter-task communication**.
- FreeRTOS queues are implemented using **FIFO (First In, First Out)** order.

#### **Key Concepts:**
- **Queues** allow you to **send messages** between tasks in a thread-safe manner.
- A queue has a **fixed size**, and if the queue is full, the writing task may block or discard data, depending on the configuration.
- FreeRTOS **queues** are ideal for scenarios where tasks need to communicate asynchronously.

---

### **Practical: Creating and Using a FreeRTOS Queue**

#### **Objective:**
- Set up two tasks: one that **sends data** (produces data) and another that **receives data** (consumes data) through a **queue**.
- In this example, the sender will send an **integer** value to the receiver task via the queue.

---

### **Steps to Implement:**
1. **Create a Queue** to hold data between tasks.
2. **Create two tasks**: one to **send data** and another to **receive data**.
3. **Send data** from the producer task to the queue.
4. **Receive data** in the consumer task from the queue.
5. **Toggle an LED** whenever the consumer task receives data.

---

### **Code for FreeRTOS Queue**
```c
#include "main.h"
#include "FreeRTOS.h"
#include "task.h"
...
#define QUEUE_LENGTH 5      // Queue length
#define ITEM_SIZE sizeof(int)  // Size of each item in the queue

QueueHandle_t xQueue;  // Declare queue handle

void ProducerTask(void *pvParameters);  // Producer task function prototype
void ConsumerTask(void *pvParameters);  // Consumer task function prototype
void SystemClock_Config(void);
void MX_GPIO_Init(void);
...
int main(void)
{
    ...
    // Create the Queue with a length of 5 items
...
/* Producer Task: Sends data to the queue */
void ProducerTask(void *pvParameters)
{
    int data = 0;
    ...
    while (1)
    {
        ...
        {
            // Toggle the LED after sending data
            HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);
            data++;  // Increment data for the next send
        }
...
void ConsumerTask(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        {
            // Toggle the LED after receiving data
...
void MX_GPIO_Init(void)
{
    ...
    GPIO_InitStruct.Pin = GPIO_PIN_7;  // LED on PB7
    HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
}
...
1. **Queue Creation:**
   ...
   xQueue = xQueueCreate(QUEUE_LENGTH, ITEM_SIZE);
   ```
   - Creates a queue that can hold up to **5 items** of size **`int`**. This queue will be used to send data between tasks.
...
2. **Producer Task:**
   ...
   ```
   - The **producer** task sends an incremented **integer** value to the queue every second.
   - After sending the data, it **toggles the LED on PA5** to indicate that data has been sent.
...
3. **Consumer Task:**
   ...
   ```
   - The **consumer** task waits for data from the queue.
   - When data is received, it **toggles the LED on PB7** to indicate that data has been consumed.

4. **LED Initialization:**
   - Two **LEDs** are toggled in this example, one on **PA5** (for producer task) and another on **PB7** (for consumer task).

5. **Queue Management Functions:**
   - **`xQueueSend()`** is used to send data to the queue from the producer task.
   - **`xQueueReceive()`** is used to receive data from the queue in the consumer task.

---

### **Expected Behavior:**
- The **Producer Task** sends data (an incremented integer) to the queue every 1 second.
- The **Consumer Task** receives the data from the queue and toggles an LED.
- The two tasks will run **concurrently** and **communicate via the queue**.
...
| **Function**              | **Purpose**                                                                  |
|---------------------------|------------------------------------------------------------------------------|
| **xQueueCreate()**         | Creates a queue with a specific length and item size.                        |
| **xQueueSend()**           | Sends data to the queue (blocks if full).                                   |
| **xQueueReceive()**        | Receives data from the queue (blocks if empty).                              |
| **xQueuePeek()**           | Allows you to look at the data in the queue without removing it.             |
| **xQueueSendFromISR()**    | Sends data to the queue from an interrupt service routine (ISR).             |
| **xQueueReceiveFromISR()** | Receives data from the queue from an ISR.                                    |

---

### **Summary:**
- **Queues** are essential for inter-task communication in FreeRTOS, allowing data to be safely exchanged between tasks.
- In this example, we demonstrated how to use **queues** to pass data between a **producer** and a **consumer** task.
- This mechanism is crucial for many embedded systems applications where tasks need to communicate asynchronously.

---

### **Step 50: FreeRTOS Semaphore Functions**

#### **Theory:**
- A **semaphore** is a synchronization primitive used in FreeRTOS to control access to shared resources.
- **Semaphores** can be used to signal between tasks or between interrupt and tasks, or to manage access to shared resources like peripherals or memory.
- There are two types of semaphores:
  1. **Binary Semaphore**: Used for signaling between tasks. It only has two states (set or unset).
  2. **Counting Semaphore**: Used for managing access to a finite resource pool. It can count up to a specified limit.

#### **Key Concepts:**
- **Semaphore** operations are used for **task synchronization** and **resource management**.
- The **FreeRTOS API** allows you to **create**, **give**, and **take** semaphores.

---

### **Practical: Creating and Using FreeRTOS Semaphores**

#### **Objective:**
- Use a **binary semaphore** for task synchronization.
- One task will **give** the semaphore, and another task will **take** the semaphore, ensuring that the second task only executes when it has been given permission by the first task.

---

### **Steps to Implement:**
1. **Create a Binary Semaphore**.
2. **Create two tasks**: One will **give** the semaphore, and the other will **take** it.
3. The task that **takes** the semaphore will only execute when it has received the semaphore from the first task.

---

### **Code for FreeRTOS Semaphore**
...
int main(void)
{
    ...
    if (xSemaphore != NULL)
    {
        // Create two tasks: Task1 will give the semaphore, Task2 will take it
        xTaskCreate(Task1, "Task1", 128, NULL, 1, NULL);
        xTaskCreate(Task2, "Task2", 128, NULL, 1, NULL);
...
/* Task1: Gives the semaphore every 2 seconds */
void Task1(void *pvParameters)
{
    while (1)
    {
        ...
        HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);  // Toggle LED to indicate Task1 is giving the semaphore

        vTaskDelay(pdMS_TO_TICKS(2000));  // 2-second delay
    }
}

/* Task2: Takes the semaphore and executes when it is given */
void Task2(void *pvParameters)
{
    while (1)
    {
        // Wait for the semaphore to be given by Task1
        if (xSemaphoreTake(xSemaphore, portMAX_DELAY) == pdTRUE)
        {
            // Toggle LED to indicate Task2 is executing after receiving the semaphore
            HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_7);
        }
...
1. **Semaphore Creation:**
   ...
   xSemaphore = xSemaphoreCreateBinary();
   ```
   - Creates a binary semaphore. This semaphore will be used for synchronization between two tasks.

2. **Task1 (Giving Semaphore):**
   ```c
   void Task1(void *pvParameters)
   {
       while (1)
       {
           xSemaphoreGive(xSemaphore);  // Give the semaphore to Task2
           HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);  // Toggle LED to show Task1 gave the semaphore
           vTaskDelay(pdMS_TO_TICKS(2000));  // Delay for 2 seconds
       }
   }
   ```
   - **Task1** will give the semaphore every 2 seconds by calling **`xSemaphoreGive()`**.
   - After giving the semaphore, it toggles the **LED on PA5** to indicate that it has done so.

3. **Task2 (Taking Semaphore):**
   ```c
   void Task2(void *pvParameters)
   {
       while (1)
       {
           if (xSemaphoreTake(xSemaphore, portMAX_DELAY) == pdTRUE)
           {
               HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_7);  // Toggle LED to show Task2 received the semaphore
           }
           vTaskDelay(pdMS_TO_TICKS(1000));  // Delay for 1 second
       }
   }
   ```
   - **Task2** will **wait** for the semaphore using **`xSemaphoreTake()`**.
   - Once it successfully receives the semaphore, it toggles the **LED on PB7** to indicate that it is running.

4. **LED Initialization:**
   - Two **LEDs** are toggled: one on **PA5** (indicating Task1 has given the semaphore) and another on **PB7** (indicating Task2 is executing after receiving the semaphore).

---

### **Expected Behavior:**
- **Task1** will give the semaphore every 2 seconds, allowing **Task2** to run.
- When **Task2** receives the semaphore, it will execute and toggle an LED on **PB7**.
- The two tasks run **concurrently**, but **Task2** only executes after **Task1** gives the semaphore.
...
| **Function**               | **Purpose**                                                              |
|----------------------------|--------------------------------------------------------------------------|
| **xSemaphoreCreateBinary()** | Creates a binary semaphore (two states: set or unset).                   |
| **xSemaphoreTake()**        | Takes (or waits for) the semaphore. If the semaphore is not available, it blocks the task. |
| **xSemaphoreGive()**        | Gives the semaphore, unblocking any task that is waiting for it.        |
| **xSemaphoreGiveFromISR()** | Gives the semaphore from an interrupt service routine (ISR).            |
| **xSemaphoreTakeFromISR()** | Takes the semaphore from an ISR.                                         |

---

### **Summary:**
- **Semaphores** in FreeRTOS are used for **task synchronization** and **resource management**.
- **Binary semaphores** are particularly useful for simple signaling between tasks.
- In this example, we used a **binary semaphore** to synchronize two tasks: one that gives the semaphore and another that takes it, with **LED indicators** to show when each task is executed.

---

### **Step 51: FreeRTOS Mutex Functions**

#### **Theory:**
- A **mutex** (short for mutual exclusion) is a synchronization mechanism that prevents multiple tasks from accessing shared resources simultaneously.
- **Mutexes** are specifically used for managing access to shared resources (like peripherals, variables, or memory) in **multi-tasking** environments.
- Unlike semaphores, **mutexes** provide **priority inheritance**, which helps to avoid issues such as priority inversion.

#### **Key Concepts:**
- A mutex behaves like a **binary semaphore**, but with additional features like **priority inheritance**.
- Mutexes are useful when a task is waiting to access a critical section and other tasks should not interfere.
- **Priority Inversion** occurs when a low-priority task holds a mutex, and a higher-priority task is waiting for it, leading to inefficient task scheduling. **Priority inheritance** resolves this problem by temporarily boosting the priority of the task holding the mutex.

---

### **Practical: Creating and Using FreeRTOS Mutexes**

#### **Objective:**
- Use a **mutex** for protecting a shared resource (like a global variable) that multiple tasks may access.
- Demonstrate **priority inheritance** by having two tasks with different priorities access the shared resource using the mutex.

---

### **Steps to Implement:**
1. **Create a Mutex** to control access to a shared resource.
2. **Create two tasks** with different priorities that will try to access the shared resource (a counter).
3. Use the **mutex** to protect the shared resource and ensure that only one task can modify the counter at a time.
...
SemaphoreHandle_t xMutex;  // Declare a mutex handle
int sharedCounter = 0;     // Shared resource (counter)
...
int main(void)
{
    ...
    if (xMutex != NULL)
    {
        // Create two tasks: Task1 will increment the counter, Task2 will decrement it
        xTaskCreate(Task1, "Task1", 128, NULL, 2, NULL);  // Higher priority for Task1
        xTaskCreate(Task2, "Task2", 128, NULL, 1, NULL);  // Lower priority for Task2
...
/* Task1: Increments the shared counter */
void Task1(void *pvParameters)
{
    while (1)
    {
        if (xSemaphoreTake(xMutex, portMAX_DELAY) == pdTRUE)  // Take the mutex to protect shared resource
        {
            sharedCounter++;  // Modify the shared counter
            HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);  // Toggle LED to indicate Task1 is modifying the counter

            xSemaphoreGive(xMutex);  // Give the mutex after modifying the counter
        }
        vTaskDelay(pdMS_TO_TICKS(500));  // Delay for 500 ms
    }
}

/* Task2: Decrements the shared counter */
void Task2(void *pvParameters)
{
    while (1)
    {
        ...
        {
            sharedCounter--;  // Modify the shared counter
            HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_7);  // Toggle LED to indicate Task2 is modifying the counter
...
1. **Mutex Creation:**
   ...
   xMutex = xSemaphoreCreateMutex();
   ```
   - Creates a mutex. A mutex is used to protect shared resources from concurrent access by multiple tasks.

2. **Task1 (Incrementing Shared Resource):**
   ```c
   void Task1(void *pvParameters)
   {
       while (1)
   ...
   ```
   - **Task1** attempts to take the mutex before modifying the shared resource `sharedCounter`. Once it acquires the mutex, it increments the counter, toggles the **LED on PA5**, and releases the mutex using **`xSemaphoreGive()`**.

3. **Task2 (Decrementing Shared Resource):**
   ```c
   void Task2(void *pvParameters)
   {
       while (1)
   ...
   ```
   - **Task2** works similarly to **Task1**, but it decrements the shared counter instead of incrementing it.

4. **LED Initialization:**
   - Two **LEDs** are toggled to show when each task is modifying the shared resource:
     - **LED on PA5** indicates **Task1** is executing.
     - **LED on PB7** indicates **Task2** is executing.

---

### **Expected Behavior:**
- **Task1** (higher priority) increments the shared counter, while **Task2** (lower priority) decrements it.
- The mutex ensures that only one task can modify the shared counter at any given time.
- **Priority inheritance** helps ensure that **Task1**, being higher priority, is allowed to run and access the shared resource without being blocked by **Task2**.

---

### **FreeRTOS Mutex Functions:**

| **Function**               | **Purpose**                                                                 |
|----------------------------|-----------------------------------------------------------------------------|
| **xSemaphoreCreateMutex()** | Creates a mutex.                                                           |
| **xSemaphoreTake()**        | Takes the mutex to gain access to the critical section (shared resource).  |
| **xSemaphoreGive()**        | Releases the mutex, allowing other tasks to access the shared resource.    |
| **xSemaphoreTakeFromISR()** | Takes the mutex from an ISR.                                                |
| **xSemaphoreGiveFromISR()** | Gives the mutex from an ISR.                                                |

---

### **Summary:**
- **Mutexes** are used to protect shared resources between tasks in a FreeRTOS application.
- **Priority inheritance** resolves priority inversion by temporarily boosting the priority of a task holding a mutex.
- In this example, we used a **mutex** to synchronize two tasks that increment and decrement a shared counter, ensuring safe access to the shared resource.

---

### **Step 52: FreeRTOS Queue Functions**

#### **Theory:**
- **Queues** in FreeRTOS are used for **inter-task communication**. They allow one task to send data to another task.
- A queue is a **FIFO (First In, First Out)** buffer where tasks can send and receive data in a synchronized manner.
- Tasks can **send data to the queue** and **receive data from the queue**. This ensures data integrity and prevents direct interference between tasks.

#### **Key Concepts:**
- **Producer-Consumer Model:** One task sends data (producer), and another task receives it (consumer).
- **Blocking and Non-blocking Operations:** A task can block while waiting to receive data, or it can check for data without blocking.
- **Queue Length:** The maximum number of items a queue can hold.
- **Queue Size:** The size of each item in the queue (i.e., the number of bytes).
- **Queue Type:** A queue can be of various types based on the size of the data (e.g., byte queue, structure queue, etc.).

---

### **Practical: Using FreeRTOS Queues**

#### **Objective:**
- Demonstrate the use of FreeRTOS **queues** to send data between two tasks.
- **Task1** will produce data (e.g., an integer) and send it to the queue.
- **Task2** will consume the data from the queue and print it.

---

### **Steps to Implement:**
1. **Create a queue** that can hold integers.
2. **Create two tasks**: one for producing data and another for consuming data.
3. The **producer task** will send integers to the queue.
4. The **consumer task** will receive integers from the queue and print them.
...
void Task1(void *pvParameters);  // Task1 function prototype (Producer)
void Task2(void *pvParameters);  // Task2 function prototype (Consumer)
void SystemClock_Config(void);
void MX_GPIO_Init(void);
...
int main(void)
{
    ...
    if (xQueue != NULL)
    {
        // Create two tasks: Task1 will produce data, Task2 will consume it
        xTaskCreate(Task1, "Task1", 128, NULL, 2, NULL);  // Producer task
        xTaskCreate(Task2, "Task2", 128, NULL, 1, NULL);  // Consumer task
...
/* Task1: Producer - Produces data and sends it to the queue */
void Task1(void *pvParameters)
{
    int count = 0;
    ...
    while (1)
    {
        if (xQueueSend(xQueue, &count, portMAX_DELAY) == pdPASS)  // Send data to the queue
        {
            HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);  // Toggle LED to indicate data is produced
            count++;  // Increment the value to be sent to the queue
        }
        vTaskDelay(pdMS_TO_TICKS(1000));  // Delay for 1 second
    }
}

/* Task2: Consumer - Receives data from the queue and prints it */
void Task2(void *pvParameters)
{
    int receivedData = 0;
    ...
    while (1)
    {
        if (xQueueReceive(xQueue, &receivedData, portMAX_DELAY) == pdPASS)  // Receive data from the queue
        {
            // Print the received data (In practice, we would use a serial interface to send data)
            // For simulation, we'll just toggle a different LED
            HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_7);  // Toggle LED to indicate data is consumed
        }
        vTaskDelay(pdMS_TO_TICKS(500));  // Delay for 500 ms
    }
}
...
1. **Queue Creation:**
   ...
   xQueue = xQueueCreate(QUEUE_SIZE, sizeof(int));
   ```
   - Creates a queue that can hold up to **5 integers** (the queue size is 5, and each item is an integer).
   - The queue is used to store data that is passed between the producer and consumer tasks.
...
2. **Task1 (Producer):**
   ...
   ```
   - **Task1** sends data (an integer, `count`) to the queue using **`xQueueSend()`**.
   - After sending data to the queue, it **increments the count** and **toggles an LED** to indicate that data has been produced.
   - The task **delays for 1 second** before sending the next integer.
...
3. **Task2 (Consumer):**
   ...
   ```
   - **Task2** receives data from the queue using **`xQueueReceive()`** and processes it.
   - In this case, the task **toggles a different LED** to indicate that data has been consumed.
   - The task **delays for 500 ms** before checking the queue again.

4. **LED Initialization:**
   - Two LEDs are used to show when the tasks are producing and consuming data:
     - **LED on PA5** indicates that **Task1** is producing data.
     - **LED on PB7** indicates that **Task2** is consuming data.

---

### **Expected Behavior:**
- **Task1** sends data (integers) to the queue every 1 second.
- **Task2** receives the data from the queue and processes it every 500 ms.
- The LEDs indicate the actions of the tasks: **PA5** for producing and **PB7** for consuming data.
...
| **Function**              | **Purpose**                                                          |
|---------------------------|----------------------------------------------------------------------|
| **xQueueCreate()**         | Creates a queue with the specified size and item size.               |
| **xQueueSend()**           | Sends data to the queue.                                             |
| **xQueueReceive()**        | Receives data from the queue.                                        |
| **xQueueSendFromISR()**    | Sends data to the queue from an ISR.                                 |
| **xQueueReceiveFromISR()** | Receives data from the queue from an ISR.                            |
| **xQueuePeek()**           | Peeks the data in the queue without removing it.                     |

---

### **Summary:**
- **Queues** are used for safe and efficient inter-task communication.
- **Task1** (Producer) sends data to the queue, and **Task2** (Consumer) receives and processes it.
- The use of **`xQueueSend()`** and **`xQueueReceive()`** ensures synchronized data exchange between tasks.
  
---

### **Step 53: FreeRTOS Event Groups**

#### **Theory:**
- **Event Groups** in FreeRTOS are used for **task synchronization**.
- They allow tasks to wait for one or more events to occur before proceeding.
- An **event group** is a set of bits, where each bit represents an individual event.
- Tasks can **wait for specific events** (e.g., certain bits to be set) or **clear bits** once an event has occurred.
  
#### **Key Concepts:**
- **Setting Event Bits:** Tasks or ISRs can set specific bits to notify other tasks.
- **Waiting for Events:** Tasks can wait for one or more bits to be set in the event group before proceeding.
- **Event Bit Operations:** You can perform **AND**, **OR**, and **CLEAR** operations on event bits.

#### **Practical Example:**
- We will create two tasks: **Task1** (Producer) will **set an event bit** in the event group, and **Task2** (Consumer) will **wait for the event bit** to be set before proceeding.

---

### **Steps to Implement:**
1. **Create an event group** to hold event bits.
2. **Create two tasks**: one for setting the event bit and another for waiting for it.
3. **Task1** will set the event bit to notify Task2.
4. **Task2** will wait for the event bit before toggling an LED.
...
#define EVENT_BIT (1 << 0)  // Define the event bit (bit 0)
#define EVENT_GROUP_MASK (EVENT_BIT)

EventGroupHandle_t xEventGroup;  // Declare an event group handle
...
int main(void)
{
    ...
    if (xEventGroup != NULL)
    {
        // Create two tasks: Task1 will set an event, Task2 will wait for it
        xTaskCreate(Task1, "Task1", 128, NULL, 2, NULL);  // Producer task
        xTaskCreate(Task2, "Task2", 128, NULL, 1, NULL);  // Consumer task
...
/* Task1: Producer - Sets the event bit */
void Task1(void *pvParameters)
{
    while (1)
    {
        HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);  // Toggle LED to indicate Task1 activity
        xEventGroupSetBits(xEventGroup, EVENT_BIT);  // Set the event bit (notifying Task2)
        vTaskDelay(pdMS_TO_TICKS(1000));  // Delay for 1 second
    }
}

/* Task2: Consumer - Waits for the event bit to be set */
void Task2(void *pvParameters)
{
    EventBits_t uxBits;
    ...
    while (1)
    {
        uxBits = xEventGroupWaitBits(
            ...
            EVENT_GROUP_MASK,    // Wait for the specific event bit
            pdTRUE,              // Clear bits on exit
            pdFALSE,             // Wait for any event bit (not all)
            portMAX_DELAY);      // Block indefinitely until the event bit is set

        if ((uxBits & EVENT_BIT) != 0)  // Check if the event bit was set
        {
            HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_7);  // Toggle LED to indicate Task2 activity
        }
    }
}
...
void MX_GPIO_Init(void)
{
    ...
    GPIO_InitTypeDef GPIO_InitStruct = {0};
    GPIO_InitStruct.Pin = GPIO_PIN_5;  // LED on PA5 (for Task1)
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

    GPIO_InitStruct.Pin = GPIO_PIN_7;  // LED on PB7 (for Task2)
    HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
}
...
1. **Event Group Creation:**
   ```c
   xEventGroup = xEventGroupCreate();
   ```
   - Creates an event group to manage event bits. The event group will hold the status of individual bits, each representing a different event.
...
2. **Task1 (Producer):**
   ...
   ```
   - **Task1** sets the event bit (`EVENT_BIT`) using **`xEventGroupSetBits()`** to notify **Task2**.
   - It **toggles an LED** (PA5) to indicate that it is active and **delays for 1 second** before setting the event bit again.
...
3. **Task2 (Consumer):**
   ...
   ```
   - **Task2** waits for the event bit (`EVENT_BIT`) to be set by **Task1**.
   - It uses **`xEventGroupWaitBits()`** to block until the event bit is set. Once the bit is set, it **toggles another LED** (PB7) to indicate that it has received the event.

4. **LED Initialization:**
   - Two LEDs are used to show the actions of **Task1** and **Task2**:
     - **LED on PA5** for **Task1** activity (setting the event bit).
     - **LED on PB7** for **Task2** activity (waiting for and receiving the event bit).

---

### **Expected Behavior:**
- **Task1** will set the event bit every 1 second and toggle the LED on PA5.
- **Task2** will wait for the event bit to be set, and once it is, it will toggle the LED on PB7.
- The two tasks synchronize using the event group.
...
| **xEventGroupWaitBits()**   | Waits for one or more bits to be set in the event group.            |
| **xEventGroupClearBitsFromISR()** | Clears bits from an ISR.                                       |
| **xEventGroupSetBitsFromISR()**   | Sets bits from an ISR.                                            |

---

### **Summary:**
- **Event groups** provide efficient synchronization between tasks by using bits as flags.
- **Task1** (Producer) sets the event bit, and **Task2** (Consumer) waits for the event before proceeding.
- **`xEventGroupSetBits()`** and **`xEventGroupWaitBits()`** are key functions for event group management.

---
### **Step 54: FreeRTOS Software Timers**

#### **Theory:**
- **Software Timers** in FreeRTOS are used for tasks that need to perform an action after a specified period of time, without blocking the task that created the timer.
- They are commonly used for periodic actions or delays, and they operate in the background, allowing the main tasks to run concurrently.

#### **Key Concepts:**
1. **Timer Functions**: 
   - **`xTimerCreate()`**: Creates a timer.
   - **`xTimerStart()`**: Starts the timer.
   - **`xTimerStop()`**: Stops the timer.
   - **`xTimerChangePeriod()`**: Changes the period of the timer.
   
2. **Timer Callbacks**: 
   - A callback function is executed when the timer expires. This function runs in the context of the timer's own interrupt service routine, meaning it is executed at the time the timer period completes.

#### **Practical Example:**
We will create a software timer that toggles an LED every 1 second. The timer callback function will execute periodically to toggle the LED.

---

### **Steps to Implement:**
1. **Create a software timer** using **`xTimerCreate()`**.
2. **Start the timer** using **`xTimerStart()`**.
3. The **timer callback function** will toggle an LED every 1 second.

---

### **Code for FreeRTOS Software Timers**
...
TimerHandle_t xTimer;  // Declare a timer handle

void TimerCallback(TimerHandle_t xTimer);  // Timer callback function prototype
void SystemClock_Config(void);
void MX_GPIO_Init(void);
...
int main(void)
{
    ...
    xTimer = xTimerCreate(
        "LED_Toggle_Timer",    // Timer name
        pdMS_TO_TICKS(1000),   // Timer period in ticks (1000ms = 1 second)
        pdTRUE,                // Autoreload the timer (i.e., it will restart automatically after each expiration)
...
/* Timer callback function: Toggles an LED every 1 second */
void TimerCallback(TimerHandle_t xTimer)
{
    HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);  // Toggle LED on PA5
}
...
1. **Timer Handle Declaration:**
   ...
   ```
   - A **timer handle** is declared, which will hold the reference to the software timer.

2. **Creating the Timer:**
   ```c
   xTimer = xTimerCreate(
       "LED_Toggle_Timer",    // Timer name
       pdMS_TO_TICKS(1000),   // Timer period in ticks (1 second = 1000ms)
       pdTRUE,                // Auto-reload timer
       0,                     // Timer ID (not used here)
       TimerCallback          // Callback function
   );
   ```
   - **`xTimerCreate()`** creates a timer that will expire every 1000ms (1 second). 
   - **`pdTRUE`** means that the timer will automatically reload after each expiration (periodic timer).
   - The **callback function**, **`TimerCallback()`**, will be executed each time the timer expires.

3. **Starting the Timer:**
   ```c
   xTimerStart(xTimer, 0);  // Start the timer (with no block time)
   ```
   - **`xTimerStart()`** starts the timer immediately with no blocking time.
...
4. **Timer Callback:**
   ...
   ```
   - The **timer callback** function is executed each time the timer expires (every second in this case).
   - It **toggles an LED** on pin **PA5** to indicate the timer's expiration.

5. **GPIO Initialization:**
   - **PA5** is configured as an **output pin** to control an LED.

---

### **Expected Behavior:**
- The **timer** will toggle the LED on **PA5** every 1 second.
- The LED will blink at a rate of **1 Hz** (on/off every second).
...
| **Function**                | **Purpose**                                                              |
|-----------------------------|--------------------------------------------------------------------------|
| **xTimerCreate()**           | Creates a software timer with a specific period and callback function.    |
| **xTimerStart()**            | Starts a software timer.                                                 |
| **xTimerStop()**             | Stops a software timer.                                                  |
| **xTimerChangePeriod()**     | Changes the period of an already created timer.                          |
| **xTimerDelete()**           | Deletes a software timer and releases any allocated memory.              |
| **xTimerIsTimerActive()**    | Checks if a timer is currently active (started but not yet expired).      |

---

### **Summary:**
- **Software timers** allow tasks to perform actions after a specific period, without blocking other tasks.
- The **callback function** of the timer is executed when the timer expires, allowing periodic actions like toggling LEDs or periodic checks.
- In this example, a timer toggles an LED every second.

---

### **Step 55: FreeRTOS Queue Sets**

#### **Theory:**
A **Queue Set** is a FreeRTOS object that allows multiple queues to be managed together. It's useful when you need to wait on multiple queues or tasks at the same time, making it ideal for event-driven systems. A queue set allows you to wait for messages from any of the queues within it.

#### **Key Concepts:**
1. **Queue Sets**:
   - **`xQueueCreateSet()`**: Creates a queue set.
   - **`xQueueSelectFromSet()`**: Waits for an item from any queue in the set.
   - **Queue Set Elements** can be individual queues, semaphores, or other synchronization objects.

2. **When to Use Queue Sets**:
   - Queue sets are useful when your task needs to wait for data from multiple sources but doesn't care which one arrives first.

#### **Practical Example:**
We will create a scenario where two queues are created, and a task waits for messages from either of them using a queue set. When a message arrives, the task will process it.
...
3. **Use `xQueueSelectFromSet()`** to wait for messages from either queue.
4. **Post messages** to the queues from different tasks.

---

### **Code for FreeRTOS Queue Sets**
...
QueueSetHandle_t xQueueSet;  // Queue set handle
...
/* Task1: Sends data to Queue1 */
void vTask1(void *pvParameters)
{
    uint32_t data = 100;
    while (1)
    {
        xQueueSend(xQueue1, &data, portMAX_DELAY);  // Send data to Queue1
        vTaskDelay(pdMS_TO_TICKS(500));  // Wait for 500ms
    }
}

/* Task2: Sends data to Queue2 */
void vTask2(void *pvParameters)
{
    uint32_t data = 200;
    while (1)
    {
        xQueueSend(xQueue2, &data, portMAX_DELAY);  // Send data to Queue2
        vTaskDelay(pdMS_TO_TICKS(700));  // Wait for 700ms
    }
}

/* Task3: Waits for data from either Queue1 or Queue2 */
void vTask3(void *pvParameters)
{
    uint32_t receivedData;
    while (1)
    {
        // Wait for data from either Queue1 or Queue2
        QueueSetMemberHandle_t xQueue = xQueueSelectFromSet(xQueueSet, portMAX_DELAY);

        if (xQueue == xQueue1)
        {
            // Data arrived from Queue1
            xQueueReceive(xQueue1, &receivedData, 0);
            HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);  // Toggle LED on PA5
        }
        else if (xQueue == xQueue2)
        {
            // Data arrived from Queue2
            xQueueReceive(xQueue2, &receivedData, 0);
            HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_5);  // Toggle another LED on PB5
        }
    }
}
...
1. **Creating Queues**:
   ...
   xQueue2 = xQueueCreate(5, sizeof(uint32_t));
   ```
   - Two queues are created, each capable of holding up to 5 items of type **`uint32_t`**.

2. **Creating a Queue Set**:
   ```c
   xQueueSet = xQueueCreateSet(2);
   ```
   - A queue set is created with space for 2 queues (i.e., `xQueue1` and `xQueue2`).

3. **Adding Queues to the Set**:
   ```c
   xQueueAddToSet(xQueue1, xQueueSet);
   xQueueAddToSet(xQueue2, xQueueSet);
   ```
   - Both **queues** are added to the **queue set**, allowing the task to wait on either queue.

4. **Task 1**: Sends data to `xQueue1` every 500ms.
   ```c
   xQueueSend(xQueue1, &data, portMAX_DELAY);
   ```

5. **Task 2**: Sends data to `xQueue2` every 700ms.
   ```c
   xQueueSend(xQueue2, &data, portMAX_DELAY);
   ```

6. **Task 3**: Waits for data from either queue using **`xQueueSelectFromSet()`**.
   ```c
   xQueueSelectFromSet(xQueueSet, portMAX_DELAY);
   ```

   - **`xQueueSelectFromSet()`** will block the task until data arrives from either **`xQueue1`** or **`xQueue2`**.

7. **GPIO Initialization**:
   - **PA5** and **PB5** are configured as output pins to control two LEDs.
   - When data arrives from **Queue1**, it toggles the LED on **PA5**; for **Queue2**, it toggles the LED on **PB5**.

---

### **Expected Behavior:**
- **Task1** will send data to **Queue1** every 500ms.
- **Task2** will send data to **Queue2** every 700ms.
- **Task3** will wait for data from either **Queue1** or **Queue2**, and when data arrives, it will toggle the corresponding LED.

---

### **FreeRTOS Queue Sets Functions:**

| **Function**                  | **Purpose**                                                              |
|-------------------------------|--------------------------------------------------------------------------|
| **xQueueCreateSet()**          | Creates a queue set that can hold multiple queues or synchronization objects. |
| **xQueueAddToSet()**           | Adds a queue or other synchronization object to a queue set.             |
| **xQueueSelectFromSet()**      | Waits for data from any queue or synchronization object in the set.      |

---

### **Summary:**
Queue sets allow you to manage and wait for multiple queues (or synchronization objects) in FreeRTOS. In this example, we demonstrated how to use a queue set to wait for messages from two different queues and toggle an LED based on which queue received data.
